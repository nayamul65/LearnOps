...
import { COURSE_4_DATA, Course4Data } from "../data/coursesData";

export interface Course4DetailsData extends Course4Data {
  courseTitle?: string;
...
export const DEFAULT_COURSE4_DATA: Course4DetailsData = {
  ...COURSE_4_DATA,
  courseTitle: COURSE_4_DATA.title,
  totalEnrolled: COURSE_4_DATA.enrolledStudents,
  youtubeReviewVideo: COURSE_4_DATA.youtubeReviewUrl,
  helplineNumber: COURSE_4_DATA.helpline,
  shortDescription: COURSE_4_DATA.description,
};

interface Course4DetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEnroll?: () => void;
  initialData?: Course4DetailsData;
}

export default function Course4DetailsModal({
  isOpen,
  onClose,
  onEnroll,
  initialData = DEFAULT_COURSE4_DATA,
}: Course4DetailsModalProps) {
  /* ── ADMIN EDITABLE STATE FOR COURSE 4 ── */
  const [data, setData] = useState<Course4DetailsData>(() => {
    const saved = localStorage.getItem("learnops_course4_admin_data");
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        return {
          ...DEFAULT_COURSE4_DATA,
          ...parsed,
          badges: Array.isArray(parsed.badges) ? parsed.badges : DEFAULT_COURSE4_DATA.badges,
        };
      } catch (e) {
        return DEFAULT_COURSE4_DATA;
      }
    }
    return {
      ...DEFAULT_COURSE4_DATA,
  ...
  const [editForm, setEditForm] = useState<Course4DetailsData>(data);
  ...
  const handleAdminSave = (e: React.FormEvent) => {
    e.preventDefault();
    setData(editForm);
    localStorage.setItem("learnops_course4_admin_data", JSON.stringify(editForm));
    setIsAdminEditing(false);
    setSavedToast(true);
    setTimeout(() => setSavedToast(false), 3000);
  };

  const helplineNum = data.helpline || data.helplineNumber || "09611-678344";
  ...
  const currentBadges = data.badges && data.badges.length > 0 ? data.badges : ["ফোনিক্স ও উচ্চারণ", "সব স্তর"];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="relative max-w-4xl w-full bg-card border border-border/80 rounded-3xl overflow-hidden shadow-2xl my-auto max-h-[92vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* ── MODAL HEADER BANNER (LearnOps Green/Purple Theme for Course 4) ── */}
        <div className="bg-gradient-to-r from-purple-700 via-purple-800 to-indigo-900 p-5 sm:p-7 text-white relative shrink-0">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                {currentBadges.map((badgeText, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1.5 bg-green-400/25 text-green-200 font-extrabold text-xs px-3 py-1 rounded-full border border-green-300/30"
                  >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title="Admin Edit Course 4 Data"
        ...
        {savedToast && (
          <div className="bg-emerald-600 text-white px-4 py-2 text-center text-xs font-bold flex items-center justify-center gap-2" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
            ...
            <span>কোর্স ৪ অ্যাডমিন ডাটা সফলভাবে আপডেট করা হয়েছে!</span>
          </div>
        )}
        ...
        <div className="p-5 sm:p-8 overflow-y-auto space-y-8 flex-1">
          ...
          {isAdminEditing && (
            <div className="bg-purple-500/10 border-2 border-purple-500/40 rounded-3xl p-6 space-y-5">
              <div className="flex items-center gap-2 text-purple-800 dark:text-purple-300 font-bold border-b border-purple-500/30 pb-3" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
                <Edit3 className="w-5 h-5 text-purple-600" />
                <h3>অ্যাডমিন কন্ট্রোল প্যানেল (কোর্স ৪ ডাটা এডিটর - Course ID: 4)</h3>
              </div>
              ...
              <form onSubmit={handleAdminSave} className="space-y-4 text-xs font-semibold">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="text"
                      value={editForm.title || editForm.courseTitle || ""}
                      onChange={(e) => setEditForm({ ...editForm, title: e.target.value, courseTitle: e.target.value })}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-foreground mb-1">শিক্ষার্থী সংখ্যা (Enrolled):</label>
                    <input
                      type="text"
                      value={editForm.enrolledStudents || editForm.totalEnrolled || ""}
                      onChange={(e) => setEditForm({ ...editForm, enrolledStudents: e.target.value, totalEnrolled: e.target.value })}
                      className="w-full p-2.5 rounded-xl bg-background border border-border text-foreground font-bold"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-foreground mb-1">ইউটিউব রিভিউ ভিডিও লিংক (YouTube Review URL):</label>
                  <input
                    type="text"
                ...
                {/* Helpline Number */}
                <div>
                  <label className="block text-foreground mb-1">হেল্পলাইন নম্বর (Helpline):</label>
                  <input
                    type="text"
                ...
                {/* Short Description */}
                <div>
                  <label className="block text-foreground mb-1">সংক্ষিপ্ত বিবরণ (Description):</label>
                  <input
                    type="text"
                    value={editForm.description || editForm.shortDescription || ""}
                    onChange={(e) => setEditForm({ ...editForm, description: e.target.value, shortDescription: e.target.value })}
                ...
                <div className="pt-2 flex gap-3">
                  <button
                    type="submit"
                    className="flex-1 bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-xl shadow-md transition-all cursor-pointer flex items-center justify-center gap-2 text-sm"
                    style={{ fontFamily: "'Hind Siliguri', sans-serif" }}
                  >
                    <Save className="w-4 h-4" />
                    <span>সংরক্ষণ করুন (Save Course 4 Data)</span>
                  </button>
                </div>
              </form>
            </div>
          ...
          <div className="bg-gradient-to-br from-purple-500/10 via-indigo-500/5 to-transparent border border-purple-500/20 rounded-3xl p-6 relative overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className="inline-flex items-center gap-1.5 bg-purple-500/15 text-purple-700 dark:text-purple-300 font-bold text-xs px-3 py-1 rounded-full">
                  ...
                  <span className="inline-flex items-center gap-1.5 bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 font-bold text-xs px-3 py-1 rounded-full">
                    ...
                    <span style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>{data.enrolledStudents || data.totalEnrolled}</span>
                  </span>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-2.5 leading-relaxed" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
                  {data.detailedOverviewText || data.description}
                </p>
              </div>
              ...
              <div className="flex items-center gap-4 bg-background border border-border/80 p-3.5 rounded-2xl shadow-xs shrink-0">
                <div className="text-center px-3 border-r border-border">
                  <span className="text-lg font-extrabold text-purple-600 block" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
                    {data.duration}
                  </span>
                  <span className="text-[10px] text-muted-foreground">কোর্স মেয়াদ</span>
                </div>
                <div className="text-center px-3">
                  <span className="text-lg font-extrabold text-green-600 block" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
                    {data.price}
                  </span>
          ...
          <div className="bg-card border border-border rounded-3xl p-6 space-y-4 shadow-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {data.programHighlights.map((feature, index) => (
                <div
                  key={index}
                  className="flex items-center gap-3 p-3.5 rounded-2xl bg-purple-500/10 border border-purple-500/20 text-purple-900 dark:text-purple-200"
                >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-md">
              ...
              <div className="relative aspect-video w-full bg-black">
                <iframe
                  ...
                  title="Course 4 YouTube Review Video"
            ...
            {/* Helpline Info Box */}
            <div className="bg-gradient-to-r from-purple-500/10 via-green-500/5 to-transparent border border-purple-500/20 rounded-3xl p-5 sm:p-6 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-sm sm:text-base text-foreground" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
                    বিস্তারিত জানতে কল করুন: <span className="font-mono text-purple-600 dark:text-purple-400 font-extrabold">{helplineNum}</span>
                  </h4>
                  <p className="text-xs text-muted-foreground mt-0.5" style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
                    আমাদের ফোনিক্স মেন্টরদের সাথে সরাসরি কথা বলে ব্যাচ সময় জেনে নিন
                  </p>
                </div>
              </div>
              ...
              <a
                ...
                className="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-2xl font-bold text-xs transition-all shadow-md cursor-pointer shrink-0"
                style={{ fontFamily: "'Hind Siliguri', sans-serif" }}
              >
          ...
          <div className="bg-gradient-to-r from-purple-700 via-indigo-800 to-green-800 rounded-3xl p-6 sm:p-8 text-white space-y-6 shadow-xl relative overflow-hidden">
            ...
            <div className="flex flex-wrap items-center justify-between gap-6 relative z-10">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="bg-green-500 hover:bg-green-600 text-white font-extrabold px-6 py-3.5 rounded-2xl text-sm transition-all shadow-lg hover:scale-105 cursor-pointer flex items-center gap-2"
                  style={{ fontFamily: "'Hind Siliguri', sans-serif" }}
                >
        ...
        <div className="bg-muted/40 px-6 py-4 border-t border-border flex items-center justify-between shrink-0 text-xs text-muted-foreground">
          <span style={{ fontFamily: "'Hind Siliguri', sans-serif" }}>
            LearnOps Verified Phonics & Language Course · Course ID: 4
          </span>
          <button
            type="button"
            onClick={onClose}
